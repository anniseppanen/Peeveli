...
/** Each instance of class `GameState` represents a single state within the Peeveli variant of
  ...
  *                           of unrevealed characters only (e.g., `"_____"`; see [[Unrevealed]]).
  *                           In later states, more and more characters will be visible (e.g., `"C___O"`).
  * @param viableSolutions    all the words in the game’s vocabulary that match the `visibleWord`
...
class GameState(val missesAllowed: Int, val previousGuesses: String, val visibleWord: String, val viableSolutions: Vector[String]):
  ...
  def numberOfSolutions = this.viableSolutions.size
  ...
  override def toString =
    this.visibleWord + ", " +
      ...
      "guesses made: " + (if this.previousGuesses.isEmpty then "none" else this.previousGuesses) + ", " +
